...
// Idempotent: wipes the "Hollow Studio" org + demo user, then reseeds.
// Leaves NO running entry (every time_entries row has an end).
...
async function main() {
  ...
  console.log(
    `Seeded: 1 user, 1 org, 3 clients, 4 projects, ${insertedTasks.length} tasks, ` +
      `${insertedTags.length} tags, ${insertedEntries.length} entries (${tagRows.length} tag links), 0 running.`
  )
  await client.end()
}
