...
| `NUXT_SESSION_COOKIE_SECURE` | no | `true` | Secure flag on the session cookie. `false` only for plain-HTTP access on a trusted network; otherwise login "succeeds" but the browser drops the cookie. Other values stop startup |
